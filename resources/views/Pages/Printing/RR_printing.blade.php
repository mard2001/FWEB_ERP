...
<html lang="en">
    ...
    <body>
        ...
        @foreach ($report->rrdetails as $index => $item)
                    ...
                    <div class="d-flex justify-content-between headerDetails">
                        <div>
                            ...
                            <div class="d-flex justify-content-between" style="min-width: 500px">
                                <p class="mb-0" style="font-size: 12px;">Supplier Code:<strong class="supplierCode text-uppercase" style="margin-left: 19px">{{ $report->poincluded->SupplierCode }}</strong></p>
                            ...
                            <p class="mb-0" style="font-size: 12px;">Supplier Name:<strong class="supplierName text-uppercase" style="margin-left: 15px">{{ $report->poincluded->posupplier->SupplierName }}</strong></p>
                            <p class="mb-0" style="font-size: 12px;">Address:<span class="supplierAdd text-uppercase" style="margin-left: 50px">{{ $report->poincluded->posupplier->CompleteAddress }}</span></p>
                        </div>
                        <div>
                            ...
                            <p class="" style="font-size: 12px;">Reference:<strong class="rrRefNum" style="margin-left: 16px">{{ $report->Reference }}</strong></p>
                            <p class="mb-0" style="font-size: 12px;">Status:<span class="rrNum text-uppercase" style="margin-left: 37px">{{ $report->Status == 1 ? "Active":"Deleted" }}</span></p>
            ...
            <tr>
                ...
                <td class="text-center" style="width: 10%;">{{ $item->SKU }}</td>  
                <td class="text-start" style="width: 25%;">{{ $item->product->Description }}</td>  
                <td class="text-start" style="width: 5%;">{{ number_format($item->convertedQuantity['convertedToLargestUnit'],2) }}</td>  
                <td class="text-center" style="width: 5%;">{{ $item->convertedQuantity['uom'] }}</td>  
                <td class="text-center" style="width: 10%;">{{ $item->itemWhsCode }}</td>  
                <td class="text-end" style="width: 10%;">{{ $item->UnitPrice }}</td>  
                <td class="text-end" style="width: 10%;">{{ $item->NetVat }}</td>  
                <td class="text-end" style="width: 10%;">{{ $item->Vat }}</td>  
                <td class="text-end" style="width: 10%;">{{ $item->Gross }}</td>  
            </tr>
            @php
        ...
        @if ($pageNumber == $totalPages)
            ...
            <div class="d-flex justify-content-between signatory text-center mb-3" style="font-size: 12px;">
                <div class="preparedDiv">
                    <div>Prepared by:</div>
                    <div class="preparer ">{{ $report->PreparedBy }}</div>
                </div>
                <div class="checkedDiv">
                    <div>Checked by:</div>
                    <div class="checker ">{{ $report->PrintedBy== '' ? "Supervisor": $report->PrintedBy }}</div>
                </div>
                <div class="approvedDiv">
                    <div>Approved by:</div>
                    <div class="approver ">{{ $report->CheckedBy == '' ? "Manager" : $report->CheckedBy }}</div>
                </div>
            </div>
